...
public class Plan extends java.util.Observable {
    ...
    private Plan(Interview interview) throws LocationNotFoundException {
    ...
    // CHANGES THROUGHOUT buildActionPlan() METHOD BELOW
    /**
     * simple Factory Method Instructor note: comment out 'synchronized' to
     * restore the race condition
     */
    public static Plan buildActionPlan(Planner planner, Interview interview) {
        System.err.println("Building plan for " + interview);
        String contact = interview.firstName + " " + interview.lastName;
        Plan p = new Plan();
        ResponderInformation r = null;

        try {
            // Checking that responder is known
            // getResponder() throws ResponderNotFoundException when appropriate
            r = planner.getResponder(contact, interview.materialSpilled);

            // Checking that responder is qualified
            if (!r.capabilities().contains(Capability.chemicalResponder)) {
                throw new ResponderNotQualifiedException();
            }

            // Qualified chemical responder so preparing plan
            // Checking that area is known
            // findArea() throws LocationNotFoundException when appropriate
            planner.findArea(interview.room, interview.building);

            // Location valid so proceeding with plan
            p = new Plan(interview);
            p.setAreaInfo(planner.findArea(interview.room, interview.building));
        ...
        } catch (ResponderNotFoundException e) {
            System.err.println(e.getMessage());
            p = planner.theDefaultPlan.clone(interview);
        } catch (ResponderNotQualifiedException e) {
            System.err.println(e.getMessage());
            p = planner.theDefaultPlan.clone(interview);
